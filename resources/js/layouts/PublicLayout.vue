...
<template>
  <div class="min-h-screen bg-white text-slate-950 dark:bg-neutral-950 dark:text-white">
    <NavbarPublic :links="props.links" :on-nav="goTo" :wa-link="props.waLink" :logo-src="props.logoSrc" />

    <main>
      <slot />
    </main>

    <FooterPublic :links="props.links" :on-nav="goTo" :wa-link="props.waLink" :logo-src="props.logoSrc" />

    <!-- WhatsApp flotante (solo aquí, nunca en Welcome) -->
    <a
      :href="props.waLink"
      target="_blank"
      rel="noopener noreferrer"
      class="fixed bottom-5 right-5 z-[60] inline-flex h-12 w-12 items-center justify-center rounded-full bg-emerald-600 shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300"
      aria-label="WhatsApp"
      title="WhatsApp"
    >
      <!-- WhatsApp SVG -->
    ...
    <!-- Subir (solo aquí, nunca en Welcome) -->
    <button
      v-show="showTop"
      type="button"
      @click="scrollToTop"
      class="fixed bottom-5 left-5 z-[60] inline-flex h-12 w-12 items-center justify-center rounded-full border border-slate-200 bg-white shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 dark:border-neutral-800 dark:bg-neutral-950"
      aria-label="Subir"
      title="Subir"
    >
