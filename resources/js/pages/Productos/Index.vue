...
// =====================================
// - FULL CARDS: desktop + mobile
// - Paginación top + bottom
// - Animaciones/hover premium
// - Modales responsivos (max-w, max-h, paddings, grid adaptativo)
// - Sin tablas (ni headers tipo tabla)
...
/**
 * FIX TS/Volar: reactive wrapper para unref profundo en template
 */
const crud = reactive(
  useProductoCrud({
...
/**
 * PerPage requerido por tu PaginationLinks
 */
 const perPage = computed<number>(() => {
  ...
  const n2 = Number(items.value.meta?.per_page ?? NaN)
...
/**
 * Wrappers filtros SearchSelect (ALL <-> '')
 */
...
/**
 * ProductoMediaManager shape
 */
type MediaManagerItem = ProductoMedia & { producto_id: number; tipo: string }
const mediaManagerInitial = computed<MediaManagerItem[]>(() => {
  const p = selectedProductForMedia.value
  if (!p) return []
  return (p.medias ?? []).map((m) => ({
    ...m,
    producto_id: p.id,
    tipo: (m as any).tipo ?? 'producto',
  }))
})
...
<template>
  ...
  <AppLayout>
    <div class="px-4 py-5 sm:px-6 md:px-8 lg:px-10 xl:px-12 2xl:px-14">
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-2 text-sm text-slate-600 dark:text-zinc-400">
              ...
              <span class="font-semibold">
                mostrando <span class="font-extrabold">{{ items.data.length }}</span>
              </span>
            </div>

            <!-- Paginación TOP -->
            <div v-if="items.data.length" class="w-full sm:w-auto">
                <PaginationLinks
                    :meta="items.meta"
                    :links="items.links"
                    :per-page="perPage"
          ...
          >
            ...
            <div class="relative flex items-start justify-between gap-3 mb-4">
              ...
              <div class="relative flex-shrink-0">
                <div
                  class="h-16 w-16 sm:h-18 sm:w-18 rounded-xl overflow-hidden border border-slate-200 dark:border-white/10
            ...
            <div class="relative flex flex-wrap gap-2">
              <Button
                ...
                size="sm"
                variant="outline"
                class="flex-1 gap-2 text-xs transition-all duration-200
      ...
      <!-- Paginación BOTTOM (siempre) -->
      <div class="mt-6" v-if="items.data.length">
        <div
          class="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-white/80 dark:bg-zinc-950/40
                 p-4 shadow-lg shadow-slate-200/20 dark:shadow-black/20 backdrop-blur-sm"
        >
        <PaginationLinks
        :meta="items.meta"
        :links="items.links"
    ...
    <div v-if="crud.modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      >
        ...
        <div class="p-4 sm:p-5 overflow-y-auto max-h-[calc(86vh-128px)] sm:max-h-[calc(88vh-128px)]">
          <!-- Layout: 1 col mobile, 2 col md, 12 grid lg -->
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-12">
              <div
                class="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-slate-50/70 dark:bg-white/5
                       p-4 sm:p-5"
              >
                ...
                <!-- Previews -->
                <div v-if="crud.fotoPreviews.length" class="mt-4 grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-3">
                  <div
...
<style scoped>
/* Micro-animación suave en hover sin “marear” */
